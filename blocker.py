...
def print_info(ubi, num=None):
    ...
    try:
        ...
        for block in ubi.blocks:
            if not ubi.blocks[block].is_valid:
                ubi.blocks[block].display()
                print 'WOWWWW'
                print type(ubi.blocks[block].vid_hdr.errors)
                print type(ubi.blocks[block].ec_hdr.errors)
