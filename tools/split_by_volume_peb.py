...
if len(sys.argv) < 3:
    print """
Usage:
    split_by_volume_peb.py <input.no_oob> <output_dir>
        Cut up file assuming vtbl_rec/layout blocks are start of image.
        Data is UBI Physical Erase Blocks.
    """
    sys.exit(1)
...
i = 0
sorted_blocks = sorted(ubi_obj.blocks, key=lambda x: ubi_obj.blocks[x].peb_num)
...
for block in sorted_blocks:
    if ubi_obj.blocks[block].is_vtbl:
        ...
        if ubi_obj.blocks[block].is_valid:
            fileh[ubi_obj.blocks[block].vid_hdr.vol_id].write(ufile_obj.read_block(ubi_obj.blocks[block]))
