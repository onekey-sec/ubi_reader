...
def by_image_seq(blocks, image_seq):
    ...
    if settings.uboot_fix:
        return list(
            filter(
                lambda block: blocks[block].ec_hdr.image_seq == image_seq
                or image_seq == 0
                or blocks[block].ec_hdr.image_seq == 0,
                blocks,
            )
        )
        ...
        return list(
            filter(lambda block: blocks[block].ec_hdr.image_seq == image_seq, blocks)
        )
