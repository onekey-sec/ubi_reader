...
class ubi_file(object):
    ...
    def __init__(self, path, block_size, start_offset=0, end_offset=None):
        ...
        if (end_offset is not None) and (end_offset > file_size):
        ...
        if remainder != 0:
            error(
                self,
                "Warning",
                """end_offset - start_offset length is not block aligned,
    ...
    def read(self, size):
        ...
        try:
            ...
            self._last_read_addr = (
                self._ubi.blocks[self._blocks[leb]].file_offset
                + self._ubi.blocks[self._blocks[leb]].ec_hdr.data_offset
                + offset
            )
