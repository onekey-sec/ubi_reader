...
def extract_dents(ubifs, inodes, dent_node, path="", perms=False):
    ...
    if not is_safe_path(path, dent_node.name):
        error(
            extract_dents,
            "Warn",
            "Path traversal attempt: %s, discarding." % (dent_node.name),
        )
        return
...
def _set_file_timestamps(path, inode):
    ...
    verbose_log(
        _set_file_timestamps,
        "timestamps: access: %s, modify: %s, path: %s"
...
def _process_reg_file(ubifs, inode, path):
    try:
        ...
        error(
            _process_reg_file,
            "Warn",
            "inode num:%s path:%s :%s" % (inode["ino"].key["ino_num"], path, e),
        )
